...
# Install verify-your-work rule for AI agent tools.
...
install_agents() {
    local target="$1"
    if [[ -f "$target" ]]; then
        if grep -q "Verify Your Work" "$target" 2>/dev/null; then
            echo "  AGENTS.md: already contains verify-your-work rule, skipping"
            return
        fi
        echo "" >> "$target"
...
install_windsurf() {
    local dir="$1"
    mkdir -p "$dir"
    cp "$FORMATS_DIR/windsurf.md" "$dir/verify-your-work.md"
    echo "  Windsurf: installed to $dir/verify-your-work.md"
}

install_cursor() {
    local dir="$1"
    mkdir -p "$dir"
    cp "$FORMATS_DIR/cursor.md" "$dir/verify-your-work.md"
    echo "  Cursor: installed to $dir/verify-your-work.md"
}
...
install_claude() {
    ...
    if [[ -f "$target" ]]; then
        if grep -q "Verify Your Work" "$target" 2>/dev/null; then
            echo "  Claude: already contains verify-your-work rule, skipping"
            return
        fi
        echo "" >> "$target"
        cat "$RULE_FILE" >> "$target"
...
echo ""
echo "Done. The verify-your-work rule is now active."
