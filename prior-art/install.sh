...
# Install prior-art rule for AI agent tools.
...
install_agents() {
    local target="$1"
    if [[ -f "$target" ]]; then
        if grep -q "Prior Art" "$target" 2>/dev/null; then
            echo "  AGENTS.md: already contains prior-art rule, skipping"
            return
        fi
        echo "" >> "$target"
...
install_windsurf() {
    local dir="$1"
    mkdir -p "$dir"
    cp "$FORMATS_DIR/windsurf.md" "$dir/prior-art.md"
    echo "  Windsurf: installed to $dir/prior-art.md"
}

install_cursor() {
    local dir="$1"
    mkdir -p "$dir"
    cp "$FORMATS_DIR/cursor.md" "$dir/prior-art.md"
    echo "  Cursor: installed to $dir/prior-art.md"
}
...
install_claude() {
    ...
    if [[ -f "$target" ]]; then
        if grep -q "Prior Art" "$target" 2>/dev/null; then
            echo "  Claude: already contains prior-art rule, skipping"
            return
        fi
        echo "" >> "$target"
        cat "$RULE_FILE" >> "$target"
...
echo "Installing prior-art rule ($SCOPE, format: $FORMAT)"
echo ""
...
echo ""
echo "Done. The prior-art rule is now active."
