...
```
uv run $SKILL_DIR/scripts/generate_handoff.py --task "TASK_DESCRIPTION" [--dir OUTPUT_DIR] [--num-tasks N]
```
...
2. Run the generate command:
   ```
   uv run $SKILL_DIR/scripts/generate_handoff.py --task "USER_TASK" --dir ".tasks" --num-tasks 5
   ```
