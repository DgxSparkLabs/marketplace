...
ln -s "$(pwd)/rules/continuous-improvement/rule.md" .claude/rules/continuous-improvement.md   # symlink (live updates)
# or:
cp rules/continuous-improvement/rule.md .claude/rules/continuous-improvement.md                # copy (portable)
```
...
`rule-continuous-improvement` IS still a plugin for these platforms. Install via the platform's native marketplace surface or clone the marketplace; the rule is auto-mirrored to `.cursor/rules/`, `.windsurf/rules/`, etc.
