...
|---|---|---|---|---|
| `ClaudeCodePlatform` | `scripts/platforms.py:101-125` | all 10 constructs | none (no separate mirror) | Canonical platform; `marketplace.json` + per-plugin `.claude-plugin/plugin.json` are written by main flow. `build_plugin_json` delegates to the construct so Claude schema stays a single source of truth. |
| `CodexPlatform` | `scripts/platforms.py:128-176` | skill, mcp, hook | `.codex/` | Codex reuses our `.claude-plugin/marketplace.json` (legacy-compatible). Emits `.codex-plugin/plugin.json` per supported plugin and mirrors skill content to `.codex/skills/`. |
| `GeminiPlatform` | `scripts/platforms.py:179-219` | skill | `.gemini/` | Mirrors skills to `.gemini/skills/`; emits the repo-level `.gemini/gemini-extension.json` via a separate `emit_extension_manifest` method (Phase 4). Returns `{}` from `build_plugin_json` — Gemini's install unit is extensions, not plugins. |
| `CursorPlatform` | `scripts/platforms.py:222-253` | rule, skill | `.cursor/` | Emits `.cursor/rules/<name>.md` from per-rule format files, plus a minimal `.cursor-plugin/plugin.json` per supported plugin. Skills are served from `.agents/` (via `AgentsPlatform`); no `.cursor/skills/` needed. |
| `WindsurfPlatform` | `scripts/platforms.py:256-280` | rule | `.windsurf/` | Emits `.windsurf/rules/<name>.md` with `trigger:` frontmatter. No CLI exists; `build_plugin_json` returns `{}`. |
| `DevinPlatform` | `scripts/platforms.py:283-306` | skill | `.devin/` | Mirrors skills to `.devin/skills/`. Devin reads rules from `.cursor/rules/` and `.windsurf/rules/` natively — no separate `.devin/rules/` emission. |
| `AgentsPlatform` | `scripts/platforms.py:309-339` | skill | `.agents/` | The `.agents/skills/<name>/SKILL.md` path is read natively by Windsurf, Cursor, AND Devin — a true cross-platform skill convergence. Implemented as a proper Platform class (not a special-case step) for symmetry with the other six. Hosts skill content only — `build_plugin_json` returns `{}`. |
...
| **6** | Root-level `.cursor-plugin/marketplace.json`. Cursor team-marketplace import expects a multi-plugin manifest at the repo root listing all Cursor-supported plugins. | `scripts/generate_manifest.py:214-236` | `.cursor-plugin/marketplace.json` at repo root |
