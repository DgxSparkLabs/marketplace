...
install_agents() {
    ...
    if [[ -f "$target" ]]; then
        if grep -q "Python UV" "$target" 2>/dev/null; then
            echo "  AGENTS.md: already contains python-uv rule, skipping"
            return
        fi
        echo "" >> "$target"
...
install_windsurf() {
    local dir="$1"
    mkdir -p "$dir"
    cp "$FORMATS_DIR/windsurf.md" "$dir/python-uv.md"
    echo "  Windsurf: installed to $dir/python-uv.md"
}

install_cursor() {
    local dir="$1"
    mkdir -p "$dir"
    cp "$FORMATS_DIR/cursor.md" "$dir/python-uv.md"
...
echo ""
echo "Done. The python-uv rule is now active."
