...
- [[README]] — user-facing install + Quick Start.
- [[HANDOFF]] — long-form project state tracker (5 completed phases, layout, next steps).
- [[docs/RESUME_HERE|RESUME_HERE]] — 30-second re-entry orientation.
- [[CONTRIBUTING]] — contributor workflow + conventions + output discipline (the canonical "how do I work in this repo" doc — see it before duplicating content here).
- [[docs/ARCHITECTURE|ARCHITECTURE]] — generator architecture: Construct + Platform protocols, six phases.
- [[docs/PLATFORMS|PLATFORMS]] — per-platform install/support reference.
- [[docs/ADDING_A_CONSTRUCT|ADDING_A_CONSTRUCT]] — the single contributor walkthrough for all 10 construct types.
- [[docs/CONSTRUCT_TYPES|CONSTRUCT_TYPES]] — reference table for the 10 constructs.

For project-wide conventions and contributor discipline, see [[CONTRIBUTING]].

This file (`AGENTS.md`) is the per-project AI-agent rule set: only project-specific conventions live here. Generic discipline content (verification ladder, blast radius, document lifecycle, output discipline, autonomous persistence, etc.) used to be duplicated here; it now lives in [[CONTRIBUTING]] under "Output discipline (...)" and the contributor's user-global content. Don't re-import generic rules into this file — keep it project-scoped.
